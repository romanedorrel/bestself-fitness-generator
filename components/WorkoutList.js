...
export default function WorkoutList() {
  ...
  const handleFormSubmit = (
    ...
    if (selectedWorkout === "Back") {
      filteredList = workouts.filter((workout) =>
        workout.primaryMuscles.some((muscle) => backBicep.includes(muscle)),
      );
    } else if (selectedWorkout === "Legs") {
      filteredList = workouts.filter((workout) =>
        workout.primaryMuscles.some((muscle) => legsGlutes.includes(muscle)),
      );
    } else if (selectedWorkout === "Chest") {
      filteredList = workouts.filter((workout) =>
        workout.primaryMuscles.some((muscle) => chestTricep.includes(muscle)),
      );
    } else if (selectedWorkout === "Shoulders") {
      filteredList = workouts.filter((workout) =>
        workout.primaryMuscles.some((muscle) =>
          shouldersTraps.includes(muscle),
        ),
      );
      ...
      filteredList = workouts.filter((workout) =>
        workout.primaryMuscles.includes(selectedWorkout),
      );
    }
    //filter the results of completed filter to then display desired fitness level
    displayList = filteredList.filter((result) =>
      result.level.includes(fitnessLevel),
    );
  ...
  const handleSave = async () => {
    ...
    const workoutData = {
      workoutName: "Workout: " + formSelected.focus,
